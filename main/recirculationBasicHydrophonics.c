...
void app_main(void) {
  // Create a configuration struct with the desired parameters
  flow_sensor_config_t config = {
      .flow_sensor_pin = GPIO_NUM_4,        // Set the GPIO pin connected to the sensor
  ...
  while (1) {
    ...
    float flow_rate = (float)get_flow_frequency() / config.pulse_to_liters_factor;
    ESP_LOGI("FlowRate", "Flow rate: %.2f L/min", flow_rate);
