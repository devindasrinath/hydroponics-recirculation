...
void start_state(state_dto_t *state_dto) {
  ESP_LOGI(TAG, "Start Main recirculation...........");
...
void turn_on_tower_flow_state(state_dto_t *state_dto) {
  ESP_LOGI(TAG, "Turning on Tower Flow. waiting for 60s for stable tower flow");
...
void end_state(state_dto_t *state_dto) {
  ...
  ESP_LOGI(TAG, "Main recirculation cycle completed");
...
void shutdown_state(state_dto_t *state_dto) {
  ESP_LOGI(TAG, "Main recirculation will be shut down.");
  //is_first_time(STATE_END, 0)==1?esp_mqtt_client_publish(client, "/topic/main_recirculation", "Process Completed", 0, 0, 0):0;
  state_dto->state = STATE_SYSTEM_SHUTDOWN;
  state_dto->error = ESP_OK;
}
